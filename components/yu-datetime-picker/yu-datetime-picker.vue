<template>
	<view class="yu-datetime-picker">
		...
		<view class="yu-datetime-mask show" :style="showUp ? '' : 'display:none!important'" @tap.stop="maskClick" @touchmove.stop.prevent catchtouchmove="true"></view>
		<view class="yu-datetime-content show" :style="showUp ? '' : 'display:none!important'">
			<view class="yu-datetime-hd" @touchmove.stop.prevent catchtouchmove="true">
				<view class="yu-datetime-btn" @tap.stop="pickerCancel">取消</view>
				<view class="yu-datetime-btn" :style="{color: color}" @tap.stop="pickerConfirm">确定</view>
			</view>
				...
				if (_this.value) {
					...
					if (_this.isAll) {
						...
						if ((valueDateArr[0] - _this.startYear) >= 0) {
							...
							_this.selectArr = [_this.formatNum(_this.startYear), valueDateArr[1], valueDateArr[2], valueTimeArr[0],valueTimeArr[1], 0];
							_this.selectRes = `${this.formatNum(_this.startYear)+'-'+valueDateArr[1]+'-'+valueDateArr[2]+' '+valueTimeArr[0]+':'+valueTimeArr[1]+':00'}`;
						}
					} else {
