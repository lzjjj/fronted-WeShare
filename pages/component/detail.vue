<template>
	<view>
		<view style="margin: 10px;">
			...
			<button v-if="type == 1 && (topic.status=='new' || topic.status=='deadline')" class="bg-gradual-blue cu-btn apply-button" @click="cancelRegister">取消报名</button>
			<view v-if="type == 2" class="flex justify-around">
				<button v-if="topic.status == 'new' || topic.status=='deadline'" class="bg-gradual-blue cu-btn apply-button" style="margin-right: 5%;" @click="cancelTopic">取消分享</button>
